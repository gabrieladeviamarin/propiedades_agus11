...
if(isset($_POST['delete'])){
    ...
    $delete = $con->prepare("UPDATE propiedades SET id_estado = 2, observacion = :observacion WHERE cod_lugar = :cod_lugar");
    $delete->bindParam(':observacion', $observacion);
    $delete->bindParam(':cod_lugar', $cod_lugar);
    $delete->execute();
...
$conteo = $con->prepare("SELECT COUNT(*) FROM propiedades WHERE id_estado = 1");
$conteo->execute();
                    ...
                    <?php foreach($propiedades as $propiedad): ?>
                        <tr>
                            ...
                            <td>
                                ...
                                |
                                <a href="javascript:void(0);" class="btn_editar" onclick='confirmar_delete(<?= json_encode($propiedad["cod_lugar"]) ?>, <?= json_encode($propiedad["nom_lugar"]) ?>)'>
...
<!----------------      ------------------------------ SEGURO (SOLO PARA ESCRITURA) ------------------------------------------------>
                              <div class="mb-3 escritura">
                                  <label for="seg" class="form-label">¿Posee seguro?</label>
                                  <div>
                                      <div class="form-check-inline">
                                          <input class="form-check-input" type="radio" name="tiene_seguro" id="seguro_si" value="1">
                                          <label class="form-check-label" for="seguro_si">Si</label>
                                      </div>
                                      <div class="form-check-inline">
                                          <input class="form-check-input" type="radio" name="tiene_seguro" id="seguro_no" value="2">
                                          <label class="form-check-label" for="seguro_no">No</label>
                                      </div>
                                  </div>
                              </div>
                              <div class="mb-3 seguro_fields">
                                  <label for="codigo_seguro" class="form-label">Código del Seguro</label>
                                  <input type="text" class="form-control" id="codigo_seguro" name="codigo_seguro">
                              </div>
                              <div class="mb-3 seguro_fields">
                                  <label for="precio_seguro" class="form-label">Precio del Seguro</label>
                                  <input type="number" min="0" step="0.01" class="form-control" id="precio_seguro" name="precio_seguro">
                              </div>
    ...
    function confirmar_delete(codigo, nombre) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // Segundo modal para solicitar la observación
                Swal.fire({
                    title: 'Motivo de inhabilitación',
                    html: `
                        <label for="observacion_delete" class="form-label" style="display: block; text-align: center; margin-bottom: 8px;">
                            Ingrese el motivo por el cual se inhabilita la propiedad:
                        </label>
                        <textarea 
                            id="observacion_delete" 
                            class="swal2-textarea" 
                            placeholder="Ejemplo: Vendida, En reparación, etc." 
                            maxlength="500"
                            style="width:90%; min-height:100px; resize:vertical; display:block; margin:0 auto;"
                        ></textarea>
                    ...
                    reverseButtons: true,
                    didOpen: () => {
                        const textarea = document.getElementById('observacion_delete');
                    ...
                    preConfirm: () => {
                        const observacion = document.getElementById('observacion_delete').value;
                        if (!observacion.trim()) {
                            Swal.showValidationMessage('Debe ingresar un motivo para inhabilitar la propiedad');
                            return false;
                        }
                    ...
                    if (result.isConfirmed) {
                        ...
                        fetch('propiedades.php', {
                            ...
                            body: `delete=1&cod_lugar=${encodeURIComponent(codigo)}&observacion=${encodeURIComponent(observacion)}`
                        })
                        ...
                        .then(data => {
                            if(data.success) {
                                Swal.fire({
                                    icon: 'success',
                                    title: 'Propiedad inhabilitada',
                                    text: 'La propiedad ha sido inhabilitada correctamente'
                                }).then(() => {
                                    location.reload();
                                });
                            } else {
...
////////////////////////// CAMPOS AGG //////////////////////////////////////
        ...
        const seguro_fields = document.querySelectorAll(".seguro_fields");
        ...
        const radios_seguro = document.querySelectorAll('input[name="tiene_seguro"]');
        ...
        function desmarcarRadios() {
            ...
            radios_seguro.forEach(function(radio) {
                radio.checked = false; 
            }); 
        }
        ...
        function mostrar_campos() {
            ...
            seguro_fields.forEach(function(field) {
        ...
        // Función para mostrar/ocultar campos de seguro
        function mostrar_seguro() {
            const seguro_si = document.getElementById("seguro_si");
            
            if (seguro_si.checked) {
                seguro_fields.forEach(function(field) {
        ...
        // Ocultar campos de seguro al inicio
        seguro_fields.forEach(function(field) {
            field.style.setProperty('display', 'none', 'important');
        });
...
////////////////////// FORMULARIO ///////////////////////////////////
        ...
        function validarFormulario(event) {
            ...
            const seguro_si = document.getElementById("seguro_si");
            const seguro_no = document.getElementById("seguro_no");
            const codigo_seguro = document.getElementById("codigo_seguro");
            const precio_seguro = document.getElementById("precio_seguro");
            ...
            if (tipo === "1") {
                ...
                if (!nro_matricula.value.trim() || !ficha_catastral.value.trim() || !valor_escritura.value.trim() || !valor_contable_l.value || !valor_contable_b.value || !valor_avaluo_l.value || !valor_avaluo_b.value || !fecha_registro.value.trim()) {
                    ...
                    // Validación de seguro (solo para tipo escritura)
                    if (!seguro_si.checked && !seguro_no.checked) {
                        Swal.fire({
                            icon: "error",
                            text: "Debe seleccionar si la propiedad tiene o no seguro"
                        });
                        valido = false;
                    }

                    if (seguro_si.checked) {
                        if (!codigo_seguro.value.trim() || !precio_seguro.value.trim()) {
                            Swal.fire({
                                icon: "error",
                                text: "Debe completar los campos del seguro"
                            });
                            valido = false;
                        } else if (parseFloat(precio_seguro.value) < 0) {
                            Swal.fire({
                                icon: "error",
                                text: "El precio del seguro debe ser mayor o igual a 0"
                            });
                            valido = false;
                        }
                    }
            ...
            if (tipo  === "4" || tipo === "5" || tipo === "2") {
                if (!nro_contrato.value.trim() || !valor_contrato.value.trim()) {
                    ...
                    valido = false;
                }  else if (n_contrato < 0 || v_contrato < 0) {
                    Swal.fire({
                        icon: "error",
                        text: "Los numeros deben ser mayor o igual a 0"
                    });
